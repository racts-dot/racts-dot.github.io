/* Pull down to refresh - the shared one for every app on this site.
   Rachel, 16 Sep 2026, from a home-screen app on her phone: "No drag down to refresh." An app saved to the
   Home Screen has no address bar and no reload button, and iOS gives it no pull-to-refresh of its own.
   Asked for again 19 Sep ("no drop to refresh"), which is when it stopped being copied per app and became
   this one file. It is a kit piece: load it, do not reimplement it.

   At the top of the page, drag down. A pill follows the finger; past the line it says "Release to refresh".
   If the page has its own Refresh button it is clicked; otherwise the service worker is asked for a newer
   copy and the page reloads. It never starts inside an open card, a text box being typed in, the floating
   buttons or the bottom bar, never when scrolled down, and a sideways swipe is left to swipe.js. */
(function () {
  "use strict";
  if (window.__appPull) return;
  window.__appPull = true;
  ...
  css.textContent =
    ...
    ".app-pull{position:fixed;left:50%;top:0;z-index:60;display:flex;align-items:center;gap:.5rem;" +
    ...
    ".app-pull.go{transition:transform .2s ease,opacity .2s ease}" +
    ".app-pull svg{width:1.1rem;height:1.1rem;flex:0 0 auto;transition:transform .15s ease}" +
    ".app-pull.ready svg{transform:rotate(180deg)}" +
    ".app-pull.busy svg{animation:app-spin .8s linear infinite}" +
    "@keyframes app-spin{to{transform:rotate(360deg)}}" +
    "@media (prefers-reduced-motion:reduce){.app-pull.busy svg{animation:none}}";
  document.head.appendChild(css);

  var pill = document.createElement("div");
  pill.className = "app-pull";
  ...
  function refresh() {
    ...
    // On an open Notion page the app already has a Refresh that re-fetches it from Notion. Use that: it is
    // what she means by refreshing, and it keeps her place instead of throwing her back to the shell.
    var own = document.querySelector("#refresh");
    if (own) {
      done = true;
      try { own.click(); } catch (e) { done = false; }
      if (done) {
        setTimeout(function () {
          state = "idle";
  ...
  document.addEventListener("touchstart", function (e) {
    ...
    if (tg.closest && tg.closest("[contenteditable],nav.dock,nav.ph-bar,header.bar,.bottombar,.ts-fab,.fb-fab,.sa-fab,.ts-panel,.fb-panel,.sa-panel")) return;
